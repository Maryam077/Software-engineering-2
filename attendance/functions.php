...
WHERE {attendance_statuses}.description = 'Absent'  AND  studentid = '" . $user_id . "'";
        ...
        foreach ($s as $key => $val) {
            if ($key == '2' || $key > '2') {
                ...
                if ($key == '1' ) {
                   echo \core\notification::add("you absent 1 time Take care", "");
               }
    ...
    public static function notify_teacher_with_absence($record) {
        ...
        $excuseid=$sp->insert("mdl_attendance_excuse", $record);
        var_dump($excuseid);
       $teacherid = $sp->teacher();
       $_POST['excuseid'] = $excuseid;
...
<form action="regist.php" method="POST">
 <input type="hidden" name="excuseid" value="'.$_POST['excuseid'].'">
