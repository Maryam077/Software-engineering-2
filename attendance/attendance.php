...
if ($mform->is_cancelled()) {
    ...
    if (!empty($fromform->status)) {
        $query = "SELECT description FROM mdl_attendance_statuses
                   Where id = '" . $fromform->status . "'";
        ...
        foreach ($s as $key => $val) {
            if ($key == 'Excused') {
                ...
                $x = $sp->execused_before($fromform->sessid, $cm->id);
                // var_dump($x);
                /*  if($x){
                  echo   \core\notification::add("you already excused", "Warning");

                  }else{ */
                $url = new moodle_url('/mod/attendance/Excused.php');
                $success = true;
                /*  } */
            } else {
